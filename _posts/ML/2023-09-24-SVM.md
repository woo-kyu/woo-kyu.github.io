...
- $\underset{w,b,\epsilon}{\textrm{min}}\frac{||w||^2}{2}$ <span style="color:skyblue">${+C\sum^N_{i=1}\epsilon_i }$</span> $\textrm{subject to ; }y_i(b+wx_i)\ge 1$ <span style="color:skyblue"> $-\epsilon_i, \epsilon_i \ge 0$ </span> $\textrm{for all }i\textrm{ inrange (1,N)}$\underset{w,b,\epsilon}{\textrm{min}}\frac{||w||^2}{2}$ <span style="color:skyblue">${+C\sum^N_{i=1}\epsilon_i }$</span> $\textrm{subject to ; }y_i(b+wx_i)\ge 1 $<span style="color:skyblue">${- \epsilon_i, ~\epsilon_i \ge 0 }$</span>$\textrm{for all } i \textrm{  inrange (1,N)}$
...
- Any function that satisfied conditions can be used as a kernel function
  ...
  - Gaussian : $K(x_i,x_j) = \textrm{exp}\bigg\{-\frac{\|x_i-x_j\|^2_{2}}{2\sigma^2}\bigg\}$
