...
- $\underset{w,b,\epsilon}{\textrm{min}}\frac{||w||^2}{2}$ <span style="color:skyblue">${+C\sum^N_{i=1}\epsilon_i }$</span> $\textrm{subject to ; }y_i(b+wx_i)\ge 1$ <span style="color:skyblue">${-\epsilon_i,~\epsilon_i\ge 0$</span> $\textrm{for all }i\textrm{ inrange (1,N)}$\underset{w,b,\epsilon}{\textrm{min}}\frac{||w||^2}{2}$ <span style="color:skyblue">${+C\sum^N_{i=1}\epsilon_i }\\ \textrm{subject to ; }y_i(b+wx_i)\ge 1 \orange{- \epsilon_i, ~\epsilon_i \ge 0 }\\ \textrm{for all } i \textrm{  inrange (1,N)}$
...
- $f(x) = w^Tx+b=\sum_{{i\in S}}\alpha_iy_i~x_i^Tx+b$
