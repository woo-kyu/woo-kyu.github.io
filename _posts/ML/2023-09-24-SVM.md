...
- P 차원에서의 Hyperplane은 P-1 차원에서의 <span style='color:orange'>Flat</span>한 Affine space (동족 좌표계)
  ...
  [Affine space](https://handhp1.tistory.com/5)

  Affine space

- Ex. 2-dimensional 에서의 hyperplane은 line, 3-dimensional 에서는 surface

<img width="351" alt="ut" src="https://github.com/woo-kyu/woo-kyu.github.io/assets/102133610/602ee73e-ee13-4d04-a34d-a0c6f37f5dfa">
...
- $\underset{b,w_{1},...,w_{p}}{\textrm{max}}M, \textrm{subject to}~M_{i}\geq M \textrm{ for all }i=1,...,N$ (subject to = 제약 조건)
  - ⇒ $\textrm{suject to }M_{i} = {\color{Orange} \frac{y_{i}(b+w_{1}x_{i 1}+...+w_{p}x_{ip})}{||w||}}$
  - ⇒ $\underset{w,b}{\textrm{max}}{\color{Orange}\frac{\hat{M}}{||w||}}$, $\textrm{subject to }y_{i}(b+w_{1}x_{i 1}+...+w_{p}x_{ip})\geq {\color{Orange} \hat{M}=M||w||}, \textrm{ for all }i=1,...,N$
  - ⇒ $\underset{w,b}{\textrm{max}}\frac{{\color{Orange}1}}{||w||}$, $\textrm{subject to }\\y_{i}(b+w_{1}x_{i 1}+...+w_{p}x_{ip})\geq {\color{Orange} 1}, \textrm{ for all }i=1,...,N$
  - $\therefore~{\color{Orange}\underset{w,b}{\textrm{min}}\frac{||w||^{2}}{2}}$, $\textrm{subject to }\\y_{i}(b+w_{1}x_{i 1}+...+w_{p}x_{ip})\geq 1, \textrm{ for all }i=1,...,N$
{% endraw %}
