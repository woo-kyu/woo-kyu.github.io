...
- Residual 은 real value와 predicted value의 차이 ($y_i-\hat f(x_i)$)로, <span style='color:orange'>negative gradient</span> 와 같은 의미
...
$\hat f_b(x_i)=\hat f_{b-1}(x_i)+v\sum^{Jb}_{j=1}\gamma^b_j \ \mathbb{I}\ (x \in R^b_j)${: .align-center}
