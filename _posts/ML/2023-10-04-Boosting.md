...
> Boosting 은 약한 학습기(weak learners)를 순차적으로 훈련시키면서, 이전 학습기가 만든 오차를 다음 학습기가 보정하도록 하여, 여러 개의 모델을 결합해 하나의 강력한 앙상블 모델을 생성하는 기법으로, 이를 통해 bias 와 variance 를 동시에 줄이려고 시도한다.
