...
wait() 및 waitpid() 함수는 부모 프로세스가 자식 프로세스의 종료 상태를 받아올 때 사용되는 시스템 호출입니다. waitpid() 함수는 pid 파라미터를 통해 원하는 프로세스나 그룹의 프로세스에 대해 특정 동작을 수행할 수 있게 해줍니다.

pid 파라미터의 값에 따른 waitpid()의 동작은 다음과 같습니다:

pid < -1:
절대값이 해당 값과 같은 프로세스 그룹 ID를 가진 모든 자식 프로세스를 기다립니다.
예시: -500을 전달하면 프로세스 그룹 500에 속한 모든 프로세스를 기다립니다.
pid == -1:
어떠한 자식 프로세스든지 기다립니다. 이는 wait() 함수와 동일한 동작을 합니다.
pid == 0:
호출하는 프로세스와 같은 프로세스 그룹에 속한 모든 자식 프로세스를 기다립니다.
pid > 0:
pid 값과 정확히 일치하는 자식 프로세스를 기다립니다.
예시: 500을 전달하면 pid가 500인 자식 프로세스를 기다립니다.
이러한 waitpid() 함수의 동작 특성은 부모 프로세스가 자식 프로세스의 종료 상태를 좀 더 세밀하게 관리하거나, 특정 조건에 맞는 프로세스 그룹의 상태를 확인하고 싶을 때 유용하게 사용될 수 있습니다.

#### waitpid() parameter
- status 파라미터:

이 파라미터는 wait()의 유일한 파라미터와 동일하게 작동합니다.
앞서 논의한 매크로를 사용하여 이 파라미터의 값에 대한 연산이 수행될 수 있습니다.
- options 파라미터:

이 파라미터는 다음 옵션 중 하나 이상을 이진 OR 연산하여 결정됩니다.

WNOHANG:
블록하지 않고, 일치하는 자식 프로세스가 이미 종료되지 않았으면 즉시 반환합니다.

WUNTRACED:
설정되면, 반환된 상태 파라미터의 WIFSTOPPED 비트가 설정됩니다. 이는 호출 프로세스가 자식 프로세스를 추적하지 않더라도 마찬가지입니다. 이 플래그는 쉘에서와 같은 더 일반적인 작업 제어의 구현을 위해 유용합니다.

WCONTINUED:
설정되면, 반환된 상태 파라미터의 WIFCONTINUED 비트가 설정됩니다. 이는 호출 프로세스가 자식 프로세스를 추적하지 않더라도 마찬가지입니다. WUNTRACED와 마찬가지로, 이 플래그는 쉘을 구현하는 데 유용합니다.
요약하자면, waitpid() 함수의 options 파라미터는 부모 프로세스가 자식 프로세스의 상태를 어떻게 조회할 것인지를 결정합니다. 이는 특히 자식 프로세스의 종료, 중지, 계속됨 상태에 따라 부모 프로세스의 반응을 세밀하게 조절할 수 있게 해줍니다.

#### status
- 성공 시:
waitpid()는 상태가 변경된 프로세스의 pid를 반환합니다.
즉, waitpid() 함수를 호출한 부모 프로세스는 이 반환 값으로 어떤 자식 프로세스의 상태가 변했는지 알 수 있습니다.
WNOHANG이 지정된 경우:
지정된 자식 또는 자식들의 상태가 아직 변경되지 않았다면, waitpid()는 0을 반환합니다.
이는 waitpid() 함수가 블로킹하지 않고 즉시 반환하는 것을 의미합니다.
- 오류 시:
waitpid()는 -1을 반환하며, 오류를 다음 세 가지 값 중 하나로 설정합니다.
ECHILD:
pid 인수로 지정된 프로세스나 프로세스들이 존재하지 않거나 호출 프로세스의 자식이 아닙니다.
EINTR:
WNOHANG 옵션이 지정되지 않았으며, 대기하는 동안 신호가 수신되었습니다.
EINVAL:
options 인수가 유효하지 않습니다.
요약하면, waitpid() 함수는 부모 프로세스에게 자식 프로세스의 상태 변경에 대한 피드백을 제공합니다. 반환 값과 오류 코드를 통해 부모 프로세스는 자식 프로세스의 상태 변화 또는 발생한 오류를 정확하게 파악할 수 있습니다.
